...
- **Stage 1 bake-off** (dev slice ~200 segments) — **AMENDED 2026-08-30 after measurement on `@mlc-ai/web-llm@0.2.84` in real Chrome.** Slate is **four** arms, not five: `Qwen3.5-2B` (2,245 MB, fastest, recommended primary), `Phi-4-mini` (3,438 MB), `Qwen3-4B` (3,432 MB), `Ministral-3-3B` (2,864 MB) — all q4f16_1 prebuilt MLC builds, each **executed end to end**, not merely present in the registry.
  ...
  **Those four numbers are VRAM, not download size, and this line used to read as though they were sizes.** They are `vram_required_MB` from `prebuiltAppConfig`, which is the only per-model figure the shipped library reports offline; `packages/tier2/src/manifest.ts` corrected the field's name to `vramRequiredMb` and this spec was not updated with it. A reader budgeting disk from them is out by more than a third: MEASURED through `navigator.storage.estimate()` on a profile holding all four arms, they occupy **7,490 MB** at that origin against 11,979 MB of VRAM figures, and on the one model measured alone a cold load of `Qwen3.5-2B` leaves 1,079 MB against its 2,245.

  **To be measured per arm:** span P/R/F1 against gold, TTFT, decode tok/s, VRAM, cold-load, plus the reporting requirements below.

  **NOT YET MEASURED, corrected 2026-09-01.** The sentence above is a REQUIREMENT and reads as a description; it had also drifted into the middle of the VRAM correction paragraph, which is not where it belongs. **No span P/R/F1 against gold — and no accuracy number of any kind — is computed anywhere in this repository.** What `apps/eval` computes and writes beside each arm's JSONL is five *run gates*: p95 TTFT, decode tok/s, span-ladder resolvable rate, duplicate rate, and non-empty-after-stop. TTFT, decode rate, VRAM (as `originStorageBytes`) and cold-load (as `engineLoadMs`/`engineWarmupMs`) are therefore measured; accuracy is not, because §2.2's boundary puts scoring in `analysis/`'s Python and that directory does not exist. `ArmGateReport.scoring.verdictMeans` points a reader at this section for the primary criterion, so the gap belongs here rather than only in the harness.
