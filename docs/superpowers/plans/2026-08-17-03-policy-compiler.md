...
**Task 9 (CLI + live Anthropic client) — OFFLINE HALF ONLY.** `test/cli.test.ts` carries the plan's 3 tests verbatim; `src/cli.ts`, `src/llm/anthropic.ts` and `scripts/record-fixtures.ts` are new. Fail-first: `Cannot find module '../src/cli.js'`. **Step 5 (the live compile) was deliberately NOT run and no compiled IR is committed** — that pass spends money and is the user's decision.

- **The installed SDK disagrees with the plan's snippet in three places, and two of them would not have been caught by a typecheck.** Checked against `@anthropic-ai/sdk@0.110.0` in the tree rather than against the skill reference the plan was written from.
  - **`zodOutputFormat` is the wrong helper for `beta.messages.parse`.** The SDK ships two: `helpers/zod` → `zodOutputFormat` → `AutoParseableOutputFormat` (pairs with the non-beta `client.messages.parse`), and `helpers/beta/zod` → `betaZodOutputFormat` → `AutoParseableBetaOutputFormat` (pairs with the beta one). The plan mixes the beta endpoint with the non-beta helper. They are structurally identical, so this compiles either way — it is a correctness-by-accident, not a type error. Uses `betaZodOutputFormat`.
  - **`fallbacks: "default"` does not typecheck.** The parameter is declared `fallbacks?: Array<BetaFallbackParam> | null`; the scalar `"default"` mode is real but unsupported by these typings. Uses the array form with an explicit `claude-opus-4-8` — chosen because Opus 4.8 carries different classifiers, so a cyber-category refusal on a policy naming credentials and private-key material genuinely recovers rather than being relabelled.
  - **The beta flag is `server-side-fallback-2026-06-01`, not `-2026-07-01`.** `AnthropicBeta` is `(string & {}) | 'literal' | ...`, so **any** string satisfies it: the plan's `-07-01` would have typechecked and failed at runtime with a 400. The `-06-01` header gates the array form and `-07-01` gates the scalar form, and pairing either with the other form is a documented 400 — so the two must move together. Both are named in a comment at the constant.
  - Confirmed present exactly as the plan assumes: `client.beta.messages.parse`, `output_config.effort`, `stop_reason === "refusal"`, and `stop_details.category`.
- **`parsed_output` was verified to infer as `T | null`, with a non-vacuous probe.** A first attempt (`const a: Want | null = r.parsed_output`) proved nothing — had inference degraded to `null`, the assignment would still compile. Re-done as a mutual-assignability check (`Eq<typeof r.parsed_output, Want | null>`), then controlled by flipping the expectation to `Want | number` and confirming it fails. Holds for both the `parse` and the `stream(...).finalMessage()` paths off one shared params object, so the plan's `schema as never` cast is not needed and was dropped.
- **Two transports, one params object.** `beta.messages.parse` at or below 16000 `max_tokens`, `beta.messages.stream(...).finalMessage()` above it — the plan's rule, implemented rather than left as a comment, since both call sites today (16000 extract, 4000 self-test) sit under the line and a future stage raising the budget would otherwise silently risk an HTTP timeout.
- **A `stop_reason === "max_tokens"` branch beyond the plan's two.** Thinking is ON BY DEFAULT on `claude-opus-5` and `max_tokens` caps thinking plus response text *together*, so a truncated structured output is a live possibility on the extraction call rather than a theoretical one. Named separately because the fix is a bigger budget or lower effort, not a schema change — which is where a bare parse failure would send a reader.
- **The response is re-validated against the caller's schema instead of `as T`.** `FixtureLlmClient` validates every replayed response; if the live client waved a response through that the fixture client would reject, `record-fixtures.ts` would write a file its own replay path refuses. The two clients must reject the same shapes.
- **The plan's third CLI test does not test what its comment says.** Its comment reads "Guards against a test or CI run silently spending money". Mutation-verified: deleting the refusal branch so the CLI falls through to `AnthropicLlmClient` by default leaves **all three plan tests green**. It passes only because this machine has no credential — the constructor throws `Could not resolve authentication method`, the CLI catches it and returns 1. On any machine with `ANTHROPIC_API_KEY` set, that same bug bills a real request and the test still cannot see it. `CliDeps` therefore gained a `liveClient` factory seam and the guard is pinned on *the factory never being called*, which holds in every environment. A companion test pins the opposite direction (`--live` does reach the seam), so a `--live` flag wired to nothing is not mistaken for a correct implementation.
- **Nine tests appended beyond the plan's three, eight of them mutation-found holes.** Each mutation passes all three plan tests and fails exactly the intended new test: the self-test corpus never written; the output basename hardcoded to `p-fin`; warnings never printed; live-by-default (above); `--live` wired to nothing; an unrecognised flag silently ignored; the `--name` traversal guard removed; and the error message emptied.
- **`--live` alongside injected fixtures is an error, not a precedence rule.** A precedence rule is something a reader has to remember correctly in order to know whether a command costs money.
- **`--name` is constrained to a plain basename.** It becomes three filenames under `--out`, so `--name ../escaped` writes outside the output directory. Mutation-verified — and the escape is not hypothetical: while mutation-testing this check the traversal really did fire, leaving `escaped.{ir.json,report.md,selftest.json}` in the shared temp root. That in turn exposed a **bug in the test itself**: it asserted on a path in the shared tmpdir, so once any run escaped, the test stayed red forever afterwards. The output directory is now nested inside a fresh `mkdtemp` so the escape target is unique per run.
- **Local work happens before the client is built.** Reading the policy, parsing the manifest and creating `--out` all precede client selection, so a typo'd path fails without having resolved credentials — which matters most in the live pass, where the alternative is paying for a run that then cannot write its output.
- **`scripts/record-fixtures.ts` runs under `vite-node`, not `tsx`.** There is no `tsx` in this tree and installing one needs the network; `vite-node` already ships as a vitest dependency, was added as an explicit devDependency, and installed **fully offline** (`pnpm install --offline`, 0 downloaded). Node 26's own type stripping cannot run the script — verified empirically: it resolves `./x.js` literally and every source file in this repo imports that way (`ERR_MODULE_NOT_FOUND: dep.js`).
- **It refuses to run without `--yes`.** It both spends money and is *destructive to the committed test basis*: the fixture key hashes the prompt, so re-recording p-fin overwrites the hand-authored `Extraction.*` / `SelfTestCases.*` files that every current assertion about entity ids, coverage and warning counts is written against. Overwriting them is the intent of this pass — Task 3 authored them explicitly as a stand-in for it — but it is not something to discover after the fact, so the gate is explicit and the header documents `git diff packages/compiler/test/fixtures/llm/` as the review.
- **It drives the real CLI rather than `compilePolicy` directly**, injecting the recorder through the `liveClient` seam, so what gets recorded is produced by exactly the code path `sih-compile --live` runs — and the compiled IR, report and self-test corpus land in `policies/compiled/` as Step 5 expects. It records the **parsed** value, which is what `FixtureLlmClient` re-validates, so recordings round-trip by construction.
- **Runnability was proven without spending anything**: the script executes under `vite-node`, resolves its full import graph (barrel → cli → anthropic → `@anthropic-ai/sdk`) and exits 2 at the `--yes` gate, which sits *before* `new AnthropicLlmClient()`. It was **not** run with `--yes`: the absence of `ANTHROPIC_API_KEY` is not proof that no credential exists, since the SDK also resolves an `ant auth login` profile.
- **Supporting edits.** `LlmSchema<T>` was added to `llm/client.ts` and exported, because `scripts/` sits at the repo root where pnpm's strict layout puts `zod` out of reach and the recorder needs the `complete` signature to wrap a client. The barrel gained `runCli`, `CliDeps` and `AnthropicLlmClient`; the compiler's `tsconfig.json` `include` gained `../../scripts` so the script is covered by the root typecheck rather than silently unchecked.
- **What a live run would do, and what stays unverifiable without one.** It would call `claude-opus-5` once per policy plus once per entityType (3 policies), write ~40+ fixtures, write nine artifacts into `policies/compiled/`, then re-run the suite to prove replay. **Nothing in `anthropic.ts` has been executed**: the model id, the beta flag, the fallback pairing, the effort level, the refusal and truncation branches, and whether 16000 tokens is actually enough for a p-fin extraction *with thinking on by default* are all verified only against the SDK's types and the API reference. The first live call is the first test of that file.
- **Task 9 result (offline half):** 409 tests green (397 before, +12 CLI: the plan's 3 plus 9 appended), 276 core / 133 compiler, typecheck clean from root across both packages, perl NUL scan clean over tracked and untracked files. Nine mutations run, each fatal to exactly the intended test. No core file touched; no compiled IR committed.
