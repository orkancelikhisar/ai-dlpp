...
**Task 7 quality review ("with fixes"; one commit, taken before Task 8 freezes streaming semantics).**

- **Left digit boundary (Important) — `surrogatePattern` becomes `` `(?<![0-9])(?:${alts})(?![0-9])` ``.** B1 fixed only the right edge, which is half the hazard: `id-number` surrogates outside the PAN shape scramble to all-digit strings, so a surrogate is routinely a bare number that can sit inside any longer figure the model writes. Probe-confirmed before the fix, with a surrogate of "8842" for a real of "1234": `"invoice 1998842 total"` → `"invoice 1991234 total"` — the REAL id's digits spliced into an unrelated number, silently, in text the user reads as authoritative. Strictly worse than the right-edge case it mirrors, since nothing about the output looks wrong. Fail-first: that one test red (`expected 'invoice 1991234 total' to be 'invoice 1998842 total'`), all 14 others green, so the lookbehind alone carries it. Verified the reviewer's trace of the four existing digit pins — every left neighbour is a space or a string start ("Vantor 2024 report" and "Vantor 22" at offset 0; "two Vantors merged" and "filed by Vantor 2 today" after a space) — so none of them can be affected by a left digit boundary, and all four still pass. Lookbehind is available on Node ≥18 and every target browser (Chrome 62+, Safari 16.4+); it is a runtime regex feature, unaffected by the package's `lib: ["ES2022"]`. Control test added so the boundary cannot silently cost the normal case: a numeric surrogate standing alone ("ref 8842 closed") still rehydrates.
- **Two silent trades pinned and documented (Minor).** (a) A digit-glued mention rehydrates to NOTHING — "Vantor2024" matches no key (the right boundary rejects it, and there is no shorter alternative), so the fake name reaches the user. Defensible and deliberate: the only alternative is matching at a digit seam, which is exactly the hazard the boundaries exist to stop. (b) Matching is case-sensitive, so a model that lowercases the surrogate ("the vantor deal") drops the rehydration; case-insensitive matching is not the fix, since it would collapse distinct surrogates onto one key and rehydrate ordinary words that collide with one. Both are now tests in a `declined rehydrations (documented trades)` block plus a numbered trade list in the `surrogatePattern` docstring. The docstring also records that left-LETTER glue stays unboundaried on purpose — "ClientVantor" → "ClientGlobex" is a rehydration worth having, not a tolerated accident — pinned by its own test as the left-edge counterpart to the pluralization pin. Framing that ties the list together: trades (a) and (b) both fail toward leaving a surrogate visible rather than emitting a wrong real value, which is the correct direction here — a stale fake name is confusing, a misattributed real one is a privacy and correctness incident.
- **Nit:** dropped the redundant `toContain(" for Globex")` in `apply.test.ts` (subsumed by the `endsWith` assertion on the next line).
- **Result:** 227 tests green (222 before, +5 rehydrate: 1 left-boundary pin, 1 numeric control, 3 declined-rehydration pins), typecheck clean, no raw NULs.

**Task 8 (streaming transform; the plan's Step 3 snippet is SUPERSEDED by two review rounds).** The plan's TEST file is authoritative and passes verbatim; the implementation below replaces the snippet. Where they disagree, this entry wins.

- **Why the snippet is superseded: it treats a match found in a partial buffer as a decision.** It does `replaceAll(tail + chunk)` and only then slices a holdback tail off the REPLACED buffer, so both edges of the digit boundary that Task 7 spent a review round installing come apart at a chunk seam. (a) *Eager prefix replacement.* With "Vantor" and "Vantor 2" both mapped — the pool-exhaustion ladder mints exactly that pair as a matter of routine, so this is the ordinary case — the chunks `["met Vantor", " 2 today"]` see a complete "Vantor" in chunk 1, replace it, and the " 2" that follows can never form "Vantor 2". The reader gets the FIRST entity's real value standing in for the SECOND entity's mention, unrecoverably. (b) *Provisional lookahead at the buffer edge.* `(?![0-9])` passes VACUOUSLY at end-of-string, so a match ending exactly at the buffer edge is a decision the next chunk still gets a vote on; `["total 8842", "1 units"]` rehydrates the id sitting inside "88421" — precisely the splice-into-an-unrelated-number hazard the left/right boundaries exist to stop, reintroduced at the seam. Fail-first evidence, snippet implemented verbatim first: (a) gave `'met Aurora Labs 2 today'` for an expected `'met Borealis Ltd today'`, (b) gave `'total 12341 units'` for an expected `'total 88421 units'`, and the digit-dense every-split-point property failed at split 6. The plan's own 5 tests were green throughout — they never put a surrogate's prefix and a digit seam on the same buffer edge, which is why the defects survived to review.
- **The settled rule (replaces "replace, then hold back").** `pending` is now a slice of the RAW, never-replaced buffer, always. Each chunk does one `matchAll` pass (never an `exec` loop — matchAll iterates a clone and leaves the instance's `lastIndex` alone) over `carry + pending + chunk`, and a match `[s, e)` may be replaced only when it is **settled**: `e < buffer.length` (its right-lookahead character was real text, so the veto is final) AND `s <= buffer.length - maxLen` (every alternative starting at `s` was short enough to be fully visible, so longest-first had its chance). The first unsettled match stops the pass; emission runs forward to `emitEnd = max(cursor, min(visibleFrom, firstUnsettledStart))` and everything from there is deferred verbatim. Two consequences worth recording. *The `min` is provably never binding* — an unsettled match has either `e == buffer.length` (so `s >= buffer.length - maxLen`) or `s > buffer.length - maxLen`, so `s >= visibleFrom` either way — but it is written out so the holdback is locally evident rather than resting on that lemma. *The plan's "known limitation" is retired, not inherited*: the snippet's docstring accepted that a real value's suffix could be re-scanned and falsely re-matched, which was a consequence of holding back replaced text; a raw tail never re-scans a replacement, exactly as `String.replace` never does, and that is what makes the every-split-point equivalence hold rather than merely usually hold.
- **The carry (left-context mechanism).** `(?<![0-9])` mirrors hazard (b) at the other edge: it also passes vacuously at position 0, so once the digit that should veto a match has been emitted, the tail alone cannot see it. The buffer keeps ONE already-emitted raw character at its head; emission starts at `base = carry.length`, never 0, so the carry is left context for the regex and is never re-emitted. One character suffices because the lookbehind inspects exactly one. Rejected the alternative of overlapping the emit regions (re-emitting a character and having the consumer de-duplicate) as pushing a correctness obligation onto every caller. No match can BEGIN at the carry — it is either the last character of a match already replaced (the canonical scan resumes after it) or a character emitted only because every match that could start there was fully visible and settled — so the `start < base` guard is an invariant check, not a code path, and is commented as such.
- **Pushback on the mandate's carry example (accepted mechanism, wrong witness).** The review specified the split `["ref 7", "8842 closed"]` as the carry pin. That split does not exercise the carry at all: the holdback retains the last `maxLen` characters, so the "7" is still sitting in `pending` when chunk 2 arrives, and the test passes with the carry deleted. The carry is only load-bearing when the emitted region ends exactly at the digit, which needs the surrogate to be flush against the END of chunk 1: `["ref 78842", " closed"]`. Both splits are pinned (the mandated one as a boundary case worth having), plus the flush variant `["ref 78842"]`, since a tail handed to `flush` mid-digit-run must still be vetoed. Mutation-verified: deleting the carry assignment leaves the mandated split green and fails the other two. The mechanism itself was right; only the witness was.
- **Test additions beyond the plan's 5 and the review's list.** (1) `["met Vantor ", "2 today"]` alongside the mandated `["met Vantor", " 2 today"]` — mutation testing showed the mandated split is caught by the right-edge rule too, so it does not isolate the longest-alternative rule; with the trailing space the short match ends before the buffer edge and only the visibility rule can defer it. Each of the three rules now has at least one test that fails when only that rule is removed. (2) A **differential property test** — 2000 seeded rounds of random map, text and chunking, asserted equal to `rehydrateText` — which is the honest statement of the transform's contract and the net for the cases nobody reasoned about. Its text is assembled from whole TOKENS, not characters, and that is load-bearing: under a uniform character alphabet a multi-character surrogate like "ABAB" is a one-in-84000 event per position and the visibility mutation survived all 2000 rounds; with tokens all three mutations die within a few hundred. (3) A **bounded-buffering test**, since the docstring's performance claim would otherwise be unpinned: `pending` never exceeds `maxLen`, so on a long non-matching stream all but a fixed window is already emitted — a streaming layer that quietly accumulates is a bug even when its output is right, and this one runs in an extension on responses of unbounded length.
- **Task 8 result:** 245 tests green (227 before, +18 rehydrate-stream: the plan's 5, 4 finality/prefix pins, 2 right-boundary, 4 left-boundary/carry, 1 differential property, 1 bounded buffering, 2 multi-surrogate incl. metacharacters), typecheck clean, no raw NULs. Barrel exports remain Task 9.
- **Task 8 quality-review fixes (supersedes the carry paragraph in the entry above):** the "no match can BEGIN at the carry / guard is an invariant check" claim was FALSE — the reviewer's counterexample (keys {AB, BC}, after AB is replaced its raw B survives as carry and a fresh matchAll finds BC at position 0) fired in 485/2000 rounds of the differential generator. Worse, the guard's skip-and-continue advanced the iterator past the skipped span, silently losing a shorter key inside it: with {AB, BC, C}, "ABC" streamed OR unchunked yielded "xC" instead of rehydrateText's "xz" — a lost rehydration the equivalence contract forbids (found via the interrupted implementer's probe; reproduced before fixing). Fix: scan ANCHORED at base via a call-local regex clone's lastIndex (matchAll honors it as the start position; lookbehind still sees the carry — lookarounds examine the subject, not the scan start), which makes carry-region matches unrepresentable and removes the guard entirely; mirrors String.replace resume semantics exactly. Also: the map is now snapshotted at construction (a caller deleting an entry mid-stream spliced literal "undefined" into a held-back match's output — pinned along with addition-ignored), and the astral-split output caveat is documented. +5 tests (3 carry-boundary resume pins incl. every-split sweep, 2 map-capture pins); {AB,BC,C} map and C/BC tokens added to the differential pool, which now catches the class continuously. 245 → 250 green.

**Task 9 pre-export gates (review-mandated, G1-G3; landed as commit 1, ahead of the barrel).** All three were "unreachable until the barrel exports it, so they gate Task 9" findings from the Task 6 review. Once `src/index.ts` names these functions the shapes are a published API, and two of the three change one.

- **G1 - `applyActions` enforces the DetectionResult span contract instead of assuming it.** The forward pass trusted spans entirely, and every violation produced a plausible result containing REAL text — the one outcome this module exists to prevent, and undetectable from outside because `applied` stays internally consistent either way. Measured before the guard: overlap `[0,10)` then `[5,8)` on "SECRETVALUE tail" gave `"[REDACTED:aws-key][REDACTED:aws-key]LUE tail"` (the cursor regressed, `slice(10,5)` is `""`, then jumped to 8, so characters 8-9 of a span the policy said to redact were re-emitted verbatim in the tail copy); `start: -4` on "hello world" gave `"hello w[REDACTED:aws-key] world"` (JS reads a negative slice index from the END, so the replacement landed in an unrelated position with the whole original surviving around it); `end: 500` on "short" gave `"[REDACTED:aws-key]"`, silently swallowing the tail. Now throws, naming the violation, the span, the entityType and the source. **Refinement on the mandate's predicate:** compared against `lastEnd` — the previous finding's end, whatever its action — rather than the rewrite `cursor`. The two are equal for rewritten spans, but `block`/`allow` leave the cursor where it was, so the mandated `f.start < cursor` is blind to a rewrite landing INSIDE a blocked span; that case mangles the blocked text and silently falsifies the G2 offsets below. Pinned by 5 tests, including that block-overlap case.
- **G2 - `ApplyResult.skipped: SkippedSpan[]`.** `applied` covers only rewritten spans, so a caller knew WHAT was blocked but not WHERE it sits in the text it renders: its offsets are into the original, and every length-changing rewrite before them moved the text, so Plan 6's review sheet would have had to replay the deltas in `applied` by hand. The forward pass knows the shift for free (`out.length + (f.start - cursor)`). `{start, end, newStart, newEnd, entityType, action: "block" | "allow"}`, same characters on both sides, disjoint from `applied` — every finding lands in exactly one list. The empty-findings identity test now reads `skipped: []`. Pinned by 3 tests (block+allow after a length-changing rewrite slice the rewritten text back to the original span text; no cross-contamination with `applied`; an unshifted span before any rewrite).
- **G3 - `normalizeFindings` validates spans.** `detect/types.ts` has always stated `text === message.slice(start, end)` and `SpanTagger` tells implementers to re-derive it, but nothing enforced it. Tier 0 cannot break it (it slices the message itself); a tokenizer-backed tier-1/2 model breaks it routinely, and everything downstream is offsets-first — `applyActions` rewrites BY SPAN, so a drifted finding sends a neighbouring word to the vault and leaves the real value sitting in the message. Now rejects out-of-range, inverted and zero-width spans (zero-width deliberately: tier 0 never emits one, and downstream it means either an empty real value the vault refuses to mint or a redaction marker spliced in where nothing was detected) and any text/slice disagreement. Needed `text` threaded into `normalizeFindings` — internal signature change, all three call sites are inside `detect`. **Neither message quotes the finding's text or the message's**, only the source, entityType, span and the two lengths: a finding's text is precisely the sensitive string this system exists to keep out of places it may be logged, and an exception message is such a place. Pinned by 5 tests (drift, out-of-range, negative start, zero-width, and the same enforcement on the tier-2 seam), one of which asserts the value does NOT appear in the message. No existing test needed changing — every hand-built finding in `orchestrator.test.ts` already used `text.slice(...)`.
- **Gate result:** 263 tests green (250 before, +13), typecheck clean.

**Task 9 (public API + end-to-end).**

- **`surrogatePattern` is deliberately NOT exported** (reviewer decision, against the plan's Step 3 snippet which lists it). The compiled alternation encodes this layer's matching rules — longest-first ordering, the digit boundary on both edges — and every one of those is a decision `rehydrateText`/`createRehydrateTransform` own rather than their callers. A caller holding the RegExp also holds a stateful `g`-flagged instance and can quietly diverge from both. Pinned by a test asserting its absence from the barrel, so it is not "helpfully" re-added.
- **`maxSurrogateLength` IS exported, with the reason recorded at the export site:** adapters need it to reason about the streaming transform's cost before wiring one up. `createRehydrateTransform` holds back up to that many characters at every chunk boundary so a surrogate split across two chunks is still matched, and that holdback is the latency a token appears to gain on its way to the user.
- **`type CryptoKey` exported alongside the five crypto helpers** (addition to the mandate's list). This package compiles under `lib: ["ES2022"]`, where `CryptoKey` is not a global type name at all — that is why `crypto.ts` derives the alias in the first place — so without it a consumer storing a vault key in a typed field (Plan 6's IndexedDB store, the session key in `chrome.storage.session`) has no way to name what `generateVaultKey` returns.
- **The plan's two e2e tests pass as written**, with one adaptation: `new Vault(new MemoryVaultStore(), SALT)` for the Task 4 D1 constructor. Nothing in the plan's assertions needed adjusting for the G1/G2 shape changes — both tests read `applied` and `text` only.
- **Four additions beyond the plan's two.** (1) `rehydrateText` end-to-end: the plan exercises only the TransformStream, and the whole-response path is what a non-SSE adapter uses. (2) A blocked PAN located through `skipped` after a pseudonymization ahead of it shifted the text — the G2 use case, through the public API rather than the unit seam. (3) An assertion on the provider-override test that `rehydrationMap` is empty for a redacted conversation: redaction is one-way by design and only pseudonymization buys the round trip. (4) The barrel-surface test described above.
- **Task 9 result:** 269 tests green (263 after the gates, +6 e2e; 250 at the start of the task), typecheck clean, no raw NULs, `pnpm test` and `pnpm typecheck` clean from the root.

**Task 9 quality review (verdict "Yes", three Minors; one follow-up commit).**

- **M1 - the span predicate is rewritten POSITIVELY, and rejects zero-width and non-numeric offsets.** G1's first form was four negated comparisons chained with `||`, which is the shape that admits exactly the values with no ordering: every comparison against NaN is false, so a non-numeric offset satisfied all four and reached `slice`. Probe-confirmed on "0123456789 tail", redacting: `start: NaN` (or a JSON-borne `null`, or an omitted field) gave `"[REDACTED:aws-key] tail"`, swallowing the ten characters before the span; `end: NaN`/`null` gave `"[REDACTED:aws-key]0123456789 tail"` — the marker emitted AND the whole original surviving after it, a LEAK wearing the shape of a successful redaction, which is worse than the swallow the review predicted; `[2.5, 6.5)` gave `"01[REDACTED:aws-key]6789 tail"`, rewriting characters 2-6 while `applied` reported 2.5 and 6.5. Zero-width `[5,5)` on "hello world" gave `"hello[REDACTED:aws-key] world"`, splicing a marker where nothing was detected. Now a sequence of positive conditions with the integer test FIRST (ordering is only total once both offsets are known to be real integers), each negated once: `Number.isInteger` both, `start >= 0`, `start !== end` (zero-width), `start < end`, `end <= text.length`, `start >= lastEnd`. Pinned by 3 tests (zero-width; five non-numeric shapes incl. `null` and `undefined`; fractional).
- **The docstring's delegation rationale was wrong on both counts and is replaced.** It claimed zero-width was left to `normalizeFindings` "at the pipeline entrance, where the text needed to judge them lives". Judging zero-width needs no text at all (`start === end`), and `applyActions` is itself an exported entrance — findings can be hand-built, replayed from storage, or deserialized from a worker message, none of which passed through `detect`. The commit that introduced the guard had already hardened it against hand-built findings for the other four classes, so the delegation was inconsistent with its own premise.
- **M1 ride-along: `normalizeFindings` gets the same integer gate** (beyond the review's scope, which assumed that predicate was already safe). It is NaN-safe by construction — the positive form fails when a comparison against NaN returns false — but it is NOT null-safe: `null` coerces to 0, so `null >= 0` is true, `null < 6` is true, and `slice(null, 6)` is `slice(0, 6)`, which means a null start cleared the range check AND the text-fidelity check and reached the merge sorting as 0. It could not leak (apply is the backstop and now rejects it), but it was diagnosed a stage late and under the wrong name. One line, same bug class, pinned by a 4-case test.
- **M2 - the `maxSurrogateLength` rationale moves to the definition site** (`rehydrate.ts`), because JSDoc on a brace-list export specifier is surfaced by neither hover nor declaration emit — so the adapter audience it is written for was the audience least likely to read it. The barrel keeps a short note that both the export and `surrogatePattern`'s absence are deliberate, and points at the full argument.
- **M3 - the skipped-span shift is pinned across TWO rewrites of differing length.** The existing test used one rewrite (-2), where "accumulates every delta" and "remembers the last one" agree. The new fixture redacts a 20-character key (-2, `[REDACTED:aws-key]` is 18) and a 10-character token (+15, `[REDACTED:generic-secret]` is 25), so the correct cumulative shift is +13 and every plausible wrong answer (-2, +15, 0) is a different number. Mutation-checked against a `newStart = f.start` mutant before being accepted.
- **Review result:** 274 tests green (269 before, +5: 3 for M1 in `apply.test.ts`, 1 for the `normalizeFindings` ride-along in `orchestrator.test.ts`, 1 for M3; M2 is documentation only). Typecheck clean, no raw NULs. Both guards now report the same phrase, `non-integer offsets`, so the pair greps as one rule.

**Whole-branch review (verdict "Ready to merge", three Minors; final commit of Plan 2).**

- **B1 - `applyActions` enforces the TEXT half of the Finding contract, not just the span half.** Same argument the span guard already rests on: `applyActions` is an exported entrance, and findings can be hand-built, replayed from storage, or deserialized from a worker message without passing through `detect`. Measured before the check, on "Globex renewed the contract." with a `[0,6)` pseudonymize finding carrying the drifted text "Globe": the outbound message was `"Ironvale renewed the contract."` — perfectly clean, no leak, because the span is fully replaced whatever `f.text` says — while the vault stored `Ironvale -> Globe` and a later turn rehydrated to `"I contacted Globe today."`, showing the user a mangled version of their own client's name one turn after the mistake. That is the whole failure: nothing observable goes wrong at send time.
- **Enforced for ALL FOUR actions, not only the branch that reads `f.text`** (the review left this to judgement). `pseudonymize` mints `f.text` as the real value, so drift corrupts the vault as above. `block`/`allow` do not read it inside `applyActions`, but `skipped` was added precisely so a review sheet can render WHAT is blocked beside WHERE it sits, and that render reads the finding's text — drift shows the user the wrong string as the reason their message cannot send, at offsets pointing elsewhere. `redact` alone is genuinely harmless: the marker is built from the entityType and `f.text` is never read. Exempting that one branch buys nothing and costs a rule that has to be re-derived on every read, and `redact` is exactly the branch a policy edit flips to `pseudonymize` — at which point a latent drift that was "harmless" becomes a vault-poisoning bug with no code change to point at. Pinned by 2 tests (the drift case, asserting the message is value-free like its neighbours; and a loop over all four actions).
- **Ordering:** the text check sits after the bounds checks and before the disjointness check — it is only meaningful once the offsets are known to slice something, and it is intrinsic to one finding where disjointness is relational.
- **B2 (doc-only) - two docstrings that predate per-winner escalation are corrected.** `strictestAction` claimed its answer "is the action every winner from that cluster carries" and `merge.ts`'s composition sketch said "action = strictest over cluster, applied to winners"; both overclaim now that `winnerAction` escalates a neverPseudonymize winner from `pseudonymize` to `redact`. One parenthetical each, pointing at `winnerAction`.
- **B3 (doc-only) - spec §5.4 gets a pointer, not a rewrite.** Two details there are superseded by what shipped: the seed-key formula (`hash(conversationId ‖ realValue)` against the salted four-part key of D1) and the holdback sketch (`maxSurrogateLen − 1`, replaced by Task 8's settled rule). Both were already logged here, but Plan 6's implementer reads the spec first. Added a one-line implementation note above the bullets sending the reader to this log; the design prose is untouched, since the design intent is not what changed.
- **Review result:** 276 tests green (274 before, +2), typecheck clean, no raw NULs.
