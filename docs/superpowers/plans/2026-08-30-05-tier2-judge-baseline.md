...
4. Parse with `parseJudgeResponse`. On failure, **one** repair retry appending the parse error; on a second failure, count `failedClosed` and emit nothing for that segment.
