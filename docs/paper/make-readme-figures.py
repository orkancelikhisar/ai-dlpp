...
THEMES = {
    "light": dict(surface="#fcfcfb", border="#e1e0d9", ink="#0b0b0b", ink2="#52514e", muted="#898781", grid="#e1e0d9", base="#c3c2b7", local="#1baf7a", hosted="#2a78d6", judge="#008300"),
    "dark":  dict(surface="#1a1a19", border="#383835", ink="#ffffff", ink2="#c3c2b7", muted="#898781", grid="#2c2c2a", base="#383835", local="#199e70", hosted="#3987e5", judge="#008300"),
}
...
if N.get("typesafe"):
    ROWS.append(("TypeSafe Jev (judgment model)", N["typesafe"]["mean"]["F1_at_half"], "judge"))
...
def understanding(t):
    ...
    b = [text(pad, 42, "Can the model tell a real client leak from a harmless mention?", 17, t["ink"], weight=600),
         ...
         swatch(pad + 200, 82, t["hosted"]), text(pad + 218, 93, "Bigger model, hosted", 12.5, t["ink2"]),
         swatch(pad + 380, 82, t["judge"]), text(pad + 398, 93, "Judgment model, hosted", 12.5, t["ink2"])]
...
assert len(DS) == 3
TS = N.get("typesafe")
TSP = None if TS is None else st.mean(TS["entity_at_half"][p]["leak_prevention"] for p in TS["passes"])
TSO = None if TS is None else st.mean(TS["entity_at_half"][p]["over_blocking"] for p in TS["passes"])
GROUPS = [("Leaks stopped", "higher is better", 100 * LOC["leak_prevention"], 100 * st.mean(d["leak_prevention"] for d in DS), None if TSP is None else 100 * TSP),
          ("Safe prompts flagged by mistake", "lower is better", 100 * LOC["over_blocking"], 100 * st.mean(d["over_blocking"] for d in DS), None if TSO is None else 100 * TSO)]
...
def prevention(t):
    ...
    b = [text(pad, 42, "How many leaks get stopped, and how many safe prompts get flagged?", 17, t["ink"], weight=600),
         ...
         swatch(pad, 82, t["local"]), text(pad + 18, 93, "Small model, in browser", 12.5, t["ink2"]),
         swatch(pad + 200, 82, t["hosted"]), text(pad + 218, 93, "Bigger model, hosted (DeepSeek)", 12.5, t["ink2"]),
         swatch(pad + 430, 82, t["judge"]), text(pad + 448, 93, "Judgment model (TypeSafe Jev)", 12.5, t["ink2"])]
    x0, x1 = 150, W - 100; pw = x1 - x0
    y = 134
    for gi, (head, hint, lv, hv, tv) in enumerate(GROUPS):
        ...
        trio = [("In browser", lv, "local"), ("Hosted LLM", hv, "hosted")] + ([("Judgment model", tv, "judge")] if tv is not None else [])
        for ri, (who, v, kind) in enumerate(trio):
            yc = g0 + 16 + ri * 28
    ...
    b.append(text(pad, y + 31, "The judgment model has a dial the others do not: turned up, it stops 78 in 100 and flags only 12.", 11.5, t["muted"]))
    label = ("Bar chart. " + "; ".join(f"{h}: in browser {round(l)} of 100, hosted {round(hv)} of 100, judgment model {round(tv)} of 100" for h, _, l, hv, tv in GROUPS) + ".")
    return svg(W, y + 48, t, b, label)
...
print("prevention:", [(h, round(l, 1), round(hv, 1), None if tv is None else round(tv, 1)) for h, _, l, hv, tv in GROUPS])
