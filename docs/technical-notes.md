...
> **This was the project README before the study was run.** The design, repository layout and run
> commands below describe the code. Status lines such as "no accuracy numbers exist" are out of
> date: the results are in the [paper](paper/paper.pdf) and the [research records](research/).
...
![WebGPU returns wrong logits on 3 of 4 loadable rungs](assets/webgpu-divergence.svg)
...
![The precision ladder is four rungs, not six](assets/model-ladder.svg)

![Tier-1 inference latency per rung and runtime](assets/latency.svg)
